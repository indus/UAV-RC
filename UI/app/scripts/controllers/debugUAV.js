...
angular.module('uavRcApp')
  .controller('DebuguavCtrl', function ($scope, socket, $timeout) {
      var slots = []
      $scope.asctecPollings = { //AscTecModel
          ASCTEC_LL_STATUS: //struct 
          {
          ...
          ASCTEC_GPS_DATA_ADVANCED: //struct 
          {
              //latitude/longitude in deg * 10ˆ7
          ...
          ASCTEC_CONTROLLER_OUTPUT: //struct 
          {
      ...
      //console.log("test");
      //UAV_ASCTEC_LL_STATUS_IS

      

      $scope.pollings = { //AscTecModel
          STATUS: //struct 
          {
              mode: "undefined", //(0 = off, 1 = on, 2 = motorsRunning, 3 = flying)
              batteryLevel: "undefined", //percentage
              timeUp: "undefined", // in sec
              timeLeft: "undefined" // esimated remaining time to fly in sec
          },
          POSITION: //struct 
      {
          //battery voltages in mV
          ...
          heading: "undefined", //optional
          accuracyHorizontal: "undefined", // in m
          accuracyVertical: "undefined" // in m
      },
      CAM_ANGLE: //struct 
      {
          pitch: "undefined", //angle in deg
          roll: "undefined" //angle in deg
          }
      }



      $scope.poll = function (evt, name) {
          evt.stopPropagation();
          var signal = "UAV_" + name + "_GET"
          console.log(signal, _.ioMsg(null, {}));
          socket.emit(signal, _.ioMsg(null, {}))
      }

      var createSlot = function (name,group) {

          var slot = "UAV_" + name + "_IS"
          slots.push(slot)
          socket.on(slot, function (msg) {
              group[name] = msg.body;
          })
      }
      for (name in $scope.pollings)
          createSlot(name, $scope.pollings)
      for (name in $scope.asctecPollings)
          createSlot(name)

      $scope.emitTarget = function (tgt) {
          console.log("UAV_TARGET_SET", _.ioMsg(null, tgt));
          socket.emit("UAV_TARGET_SET", _.ioMsg(null, tgt))
      }

      $scope.emitCam = function (cam) {
          console.log("UAV_CAM_ANGLE_SET", _.ioMsg(null, cam));
          socket.emit("UAV_CAM_ANGLE_SET", _.ioMsg(null, cam))
      }

      socket.emit('CORE_SL_SLOTS_SET', _.ioMsg(null, {slots:slots}))
  });